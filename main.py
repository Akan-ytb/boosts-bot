...
async def on_ready():
    ...
    activity = discord.Activity(type=discord.ActivityType.playing, name="discord.gg/akan")
    await bot.change_presence(activity=activity)

ALLOWED_USER_IDS = [245573481041756161]  # Replace with the actual user IDs
...
async def operate(interaction: discord.Interaction, guild_id: str, token_type: str, nickname: str = None, pfp_path: str = None):
    if not guild_id or token_type not in ['1', '3']:
        embed = discord.Embed(title="Missing or Invalid Arguments", description="Please provide the guild ID and token type (1 for 1M tokens or 3 for 3M tokens).", color=discord.Color.purple())
        embed.add_field(name="Command Usage", value="/boost <guild_id> <token_type> [nickname] [pfp_path]")
        await interaction.response.send_message(embed=embed)
        return

    embed = discord.Embed(title="Boost Operation", description=f"Starting operation for guild ID: {guild_id} with nickname: {nickname} and profile picture: {pfp_path}", color=discord.Color.purple())
    await interaction.response.send_message(embed=embed)
    ...
    embed = discord.Embed(title="Token Count", description="How many tokens would you like to use?", color=discord.Color.purple())
    ...
    try:
        ...
        embed = discord.Embed(title="File Not Found", description=f"The file {token_file} was not found.", color=discord.Color.purple())
        await interaction.followup.send(embed=embed)
        return

    # Check if there are enough tokens
    if token_count > len(tokens):
        embed = discord.Embed(title="Not Enough Tokens", description=f"You have {len(tokens)} tokens.", color=discord.Color.purple())
        await interaction.followup.send(embed=embed)
        return
...
async def add_token(interaction: discord.Interaction, token_type: str, token: str):
    if token_type not in ['1', '3']:
        embed = discord.Embed(title="Invalid Token Type", description="Please enter either '1' for 1M tokens or '3' for 3M tokens.", color=discord.Color.purple())
        await interaction.response.send_message(embed=embed)
        return
...
async def remove_token(interaction: discord.Interaction, token_type: str, token: str):
    ...
    if not token:
        embed = discord.Embed(title="Missing Token", description="Please provide the token to remove.", color=discord.Color.purple())
        await interaction.response.send_message(embed=embed)
        return
...
async def stock(interaction: discord.Interaction):
    ...
    embed = discord.Embed(title="**Server Boost Stock**", description="", color=discord.Color.purple())
    embed.set_thumbnail(url="https://cdn.discordapp.com/attachments/1126786137587990580/1297670107165753405/ezgif-2-2f13450dfc.gif?ex=6716c4f7&is=67157377&hm=a9a8b262dddc3ac3c6ae3e99ed85c3123245208813e35db107d317fa428855d0&")
    embed.set_footer(text="LegitShop, All Rights Reserved.", icon_url="https://cdn.discordapp.com/attachments/1126786137587990580/1297670107165753405/ezgif-2-2f13450dfc.gif?ex=6716c4f7&is=67157377&hm=a9a8b262dddc3ac3c6ae3e99ed85c3123245208813e35db107d317fa428855d0&")
